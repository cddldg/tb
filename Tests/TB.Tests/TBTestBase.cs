...
namespace TB.Tests
{
    public abstract class TBTestBase : AbpIntegratedTestBase<TBTestModule>
    {
        ...
        protected void UsingDbContext(Action<TBDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<TBDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<TBDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<TBDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<TBDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TBDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<TBDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TBDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<TBDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<TBDbContext, Task<T>> func)
        {
            T result;
